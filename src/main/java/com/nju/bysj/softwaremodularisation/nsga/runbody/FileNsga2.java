...
public class FileNsga2 {
	...
	public Population run(int iteration, String programPath, String objectivePath, String frontPath, String costPath) {
...
//		Reporter.reportGeneration(parent, child, generation, this.configuration.objectives, historyRecord.size());
		while(configuration.getTerminatingCriterion().shouldRun(parent, ++generation, this.configuration.getGenerations())) {
			...
			if (generation % 10 == 0) {
				...
				System.out.println("种群:" + parent.getPopulace().size());
				System.out.println("层数:" + Common.splitPopulationByNonDominatedFront(parent).size());
			}
//			Reporter.reportGeneration(parent, child, generation, this.configuration.objectives, historyRecord.size());
		}
//		System.out.println("++++++++++++");
//		System.out.println(parent.getPopulace().size());
//		System.out.println(child.getPopulace().size());
		Population tmpPopulation = this.preparePopulation(Common.combinePopulation(parent, child, this.configuration.getPopulationSize()));
//		System.out.println(tmpPopulation.getPopulace().size());

		// 最后输出最后一代的混合种群的第一前沿
		parent = this.getChildFromCombinedPopulation(tmpPopulation);
		System.out.println("最后一代-----" + parent.getPopulace().size());
		...
		List<List<Double>> evaluations = Common.evaluation(firstFront);
		double avgTurboMQ = 0;
		double avgSemantic = 0;
		double avgREI = 0;
		for(List<Double> val: evaluations) {
//			System.out.println(val);
			avgTurboMQ += val.get(0);
			avgSemantic += val.get(1);
			avgREI += val.get(2);
		}
		int solutionNums = evaluations.size();
		System.out.println("平均turboMQ: " + avgTurboMQ / solutionNums);
		System.out.println("平均semantic: " + avgSemantic / solutionNums);
		System.out.println("平均REI: " + avgREI / solutionNums);
